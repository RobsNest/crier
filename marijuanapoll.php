...
$webpage = "Recreational Marijuana";
	...
	if($phone) {
		...
		if($vote) {
			if(!isset($_COOKIE[$cookieName])) {
				$sel_stmt = $db->prepare("SELECT id, option, cnt FROM polloptions WHERE option=?");
				$sel_stmt->bind_param("s", $vote);
				$sel_stmt->execute();
				$sel_stmt->bind_result($id, $option, $cnt);
				$sel_stmt->store_result();
				$sel_stmt->fetch();
				
				$cnt++;

				#Insert into databaseA
				$ud_stmt = $db->prepare("UPDATE polloptions SET cnt=? WHERE id=?");
				$ud_stmt->bind_param("ii", $cnt, $id);
				$ud_stmt->execute();
			}
			$cookieName = "RecreationMarijuana";
			$cookieValue = $id . "|" . $vote . "|" . $today . "|" .$cnt;
...
<head>
	<title>Poll - Recreational Marijuana - The Cumberland Crier - <?php echo $today?></title>
...
purposes?<br />
<form action="marijuanapoll.php" name="theVote" method="post" 
enctype="multipart/form-data"  onsubmit="return(validate());">
<input type="hidden" name="action" value="castvote" />
...
$sql = "SELECT id, option, cnt FROM polloptions ORDER BY id DESC";
$result = $db->query($sql);
if ($result->num_rows > 0) {
	while($row = $result->fetch_assoc()) {
		if($row["viewable"] == "Yes") {
        	echo "<div class=\"row\"><div class=\"column left\"><b>" . $row["option"] . "</div><div class=\"column right\">" . $row["cnt"] . "</b></div></div>";
       	}
	}
}
